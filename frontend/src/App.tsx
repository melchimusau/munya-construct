...
import {
  ...
  ClipboardList, CalendarCheck, Package, FolderOpen,
  Briefcase, BarChart3, UserCircle, ShoppingCart, Truck,
...
function AppContent() {
  ...
  // Redirection vers le changement de mot de passe si nécessaire
  if (user?.must_change_password) {
    return <ResetPasswordView onPasswordChanged={() => window.location.reload()} />;
  }

  // Si l'utilisateur est un employé, afficher directement le portail employé (sans sidebar admin)
    ...
    <div className="flex flex-col h-full">
      <div className="p-6 border-b border-slate-800 flex items-center gap-3">
        <div className="bg-white/95 p-1.5 rounded-lg shadow-sm shrink-0">
          <img src={logoMunya} alt="Logo" className="w-20 h-auto object-contain" />
        </div>
        <div>
          <h1 className="font-bold text-lg tracking-wide">Munya construct</h1>
        </div>
      </div>
      <nav className="p-4 space-y-1 flex-1 overflow-y-auto">
        {tabs.map(tab => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${activeTab === tab.key ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
      ...
      <div className="p-4 border-t border-slate-800">
        ...
        <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-all">
          Déconnexion
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800">
      ...
      {/* Sidebar mobile */}
      <div className={`fixed top-0 left-0 z-50 h-full w-64 bg-slate-900 text-white transform transition-transform duration-300 lg:hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {sidebarContent}
      </div>
      ...
      <main className="flex-1 overflow-y-auto flex flex-col w-full lg:w-auto">
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 md:px-8 shrink-0 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <button className="lg:hidden p-2 rounded-lg hover:bg-slate-100 text-slate-700" onClick={() => setSidebarOpen(!sidebarOpen)}>
        ...
        <div className="p-4 md:p-6 lg:p-8 flex-1 max-w-7xl w-full mx-auto">
          {renderContent()}
          <div className="text-center text-xs text-slate-400 pt-8 pb-4">Munya Construct Kolwezi/Judex lupepe/Joli site/Manika</div>
        </div>
      </main>
    </div>
  );
