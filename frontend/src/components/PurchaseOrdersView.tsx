...
import { apiService, Supplier, PurchaseOrder, PurchaseOrderLine } from '../services/api';
          ...
          <div className="space-y-3 mb-6">
            {lines.map((line, index) => (
              <div key={index} className="flex flex-col sm:flex-row gap-2">
                ...
                />
                <button onClick={() => removeLine(index)} disabled={lines.length === 1} className="text-red-600 disabled:opacity-40">
                  <Trash2 size={18} />
                </button>
              </div>
            ))}
            <button onClick={addLine} className="flex items-center gap-1 text-blue-600 text-sm">
            ...
            <tbody className="text-sm">
              {purchaseOrders.map(po => (
                <tr key={po.id} className="border-b hover:bg-slate-50">
                  ...
                  <td className="py-3">
                    <span className={`px-2 py-1 rounded-md text-xs font-medium ${statusColors[po.status] || 'bg-slate-100 text-slate-600'}`}>
                      {po.status}
                    </span>
                  </td>
                </tr>
